...
if [[ $# -lt 3 ]]; then
    echo "Usage: $( basename $0 ) <image-repo> <application-name> <image-tag> <optional:skip_ecr_check>"
    exit 1
fi
...
if echo $app_image_version | grep -iq snapshot; then
  ...
  if [ -z "$skip_ecr_check" ] && utilities/find-ecr-image.sh $application_name $app_image_version >/dev/null; then
